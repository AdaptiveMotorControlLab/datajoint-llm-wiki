...
| **populate() / make()** | `populate()` calls `make(key)` for each key in `key_source` not yet present in the table. `key_source` defaults to the natural join of all upstream FK tables minus already-computed rows. `populate(reserve_jobs=True)` performs an **atomic INSERT** into the per-table `~~table_name` jobs table *before* calling `make()` — competing workers query this table and skip any key already in `reserved` state, preventing double-computation. Always use `reserve_jobs=True` with multiple workers. |
| **Three-phase make()** | For any long-running or distributed `make()`, always emit three **structurally separate methods** — not comments inside one body: `make_fetch` (I/O only), `make_compute` (CPU/GPU only), `make_insert` (DB transaction only). This keeps the DB connection closed during the slow phases. See [[ComputationModel]]. |
